...
Warto zaznaczyć, że $t_0$, T, $t_d$ będą podawane w roku użycia osoby zarabiającej/przyjmującej emeryturę. W modelu założyliśmy, że \newline $t_0$ = 0, T = 40, $t_d$ = 60, natomiast na wykresie będą to odpowiednio lata życia: $t_0$ = 22, T = 62, $t_d$ = 82 itd. W skrócie, zakładamy, że zaczynamy zarabiać w wieku 22 lat i od tego momentu możemy zbierać na emeryturę.
...
analiza_wrazliwosci <- function() {
  ...
  zakresy <- list(
    ...
    c  = seq(0.02, 0.09, 0.0005),  #oprocentowanie obligacji raczej 2% < . < 8%
    I  = seq(0.00, 0.08, 0.0005),  #podwyżki roczne < 8% ? to i tak jest dużo
    t0 = 0:23,                     #rozpoczęcie odkładania na emeryturę 22-55(rok życia)
    t  = 33:43,                    #wiek rozpoczęcia emerytury 55–65 (od mojego obecnego wieku)
...
wiek_start <- 22 #wiek startowy, aby wykresy były bardziej czytelne
...
wyniki_2$parametr <- factor(
  ...
  labels = c("Inflacja Pi (%)", "Stopa zwrotu c (%)", "Wzrost pensji I (%)", "Wiek rozpoczęcia oszczędzania", "Wiek przejścia na emeryturę", "Wiek śmierci")
)
...
#   labs(
#     title = "Wrażliwość p na parametry modelu",
#     x = "Wartość parametru",
#     y = "p (%)"
#   ) +
...
#     "Wiek rozpoczęcia oszczędzania" = "darkmagenta",
...
zakresy_hm <- expand.grid(c = seq(0.001, 0.1, 0.005), I = seq(0, 0.08, 0.005))
...
hm_c_I <- ggplot(zakresy_hm, aes(x = c_proc , y = I_proc, fill = p)) +
  geom_tile() +
  scale_fill_gradient(low = "skyblue", high = "maroon3") +
  labs(title = "Heatmapa wrażliwości p w zależności od c i I",
       x = "c (stopa zwrotu z obligacji w %) ",
       y = "I (stopa wzrostu wypłaty w %)",
       fill = "p (%)") +
  theme(
...
#Heatmapa wpływu momentu przejścia na emeryturę i śmierci na p

zakresy_t_td <- expand.grid(t = 33:43, td = 48:78)
...
hm_t_td <- ggplot(zakresy_t_td, aes(x = t, y = td, fill = p)) +
 ...
 scale_fill_viridis_c(option = "plasma") +
  labs(
    title = TeX("Mapa wrażliwości p w zależności od t i $t_d$"),
...
#Wykres zaczęcia oszczędzania na emeryturę
...
hm_t0_td <- ggplot(zakresy_t0_td, aes(x = t0, y = td, fill = p)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma") +
  labs(
    title = TeX("Mapa wrażliwości p w zależności od $t_0$ i $t_d$"),
    x = TeX("$t_0$ (wiek rozpoczęcia pracy)"),
    y = TeX("$t_d$ (wiek śmierci)"),
    fill = "p (%)"
  )


zakresy_t0_t <- expand.grid(t0 = 0:23, t = 33:43)
...
hm_t0_t <- ggplot(zakresy_t0_t, aes(x = t0, y = t, fill = p)) +
  geom_tile() +
  scale_fill_viridis_c(option = "heat") +
  labs(
    title = TeX("Mapa wrażliwości p w zależności od $t_0$ i T"),
    x = TeX("$t_0$ (wiek rozpoczęcia pracy)"),
...
```{r,fig.align='center'}
grid.arrange(hm_c_I,ncol=1)
```

\textbf {Wnioski z powyższych wykresów wrażliwości:}
\begin{itemize}[label=$\square$]
\item \textbf{Wpływ inflacji}: {Inflacja nie ma wpływu na procent odkładanej pensji, ponieważ używamy realnych stóp procentowych. Fakt ten można również wywnioskować z braku $\Pi$ we wzorze na p z $\S_1$.}
\item \textbf{Wpływ stopy zwrotu:} {fsf}
\item \textbf{Wpływ wzrostu pensji:} {sgsg}
\item \textbf{Wpływ wieku rozpoczęcia oszczędzania:} {gsg}
\item \textbf{Wpływ wieku przejścia na emeryturę:} {sgsg}
\item \textbf{Wpływ wieku śmierci:} {gsg}
\end{itemize}
...
\begin{itemize}
    ...
    \item W rzeczywistości ludzie (błędnie) polegają na systemie publicznym (ZUS) i odkładają prywatnie mniejsze kwoty, lub i nie.
\end{itemize}
