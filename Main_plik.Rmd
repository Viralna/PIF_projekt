...
Warto zaznaczyć, że $t_0$, T, $t_d$ będą podawane w roku użycia osoby zarabiającej/przyjmującej emeryturę. W modelu założyliśmy, że \newline $t_0$ = 0, T = 40, $t_d$ = 60, natomiast na wykresie będą to odpowiednio lata życia: $t_0$ = 22, T = 62, $t_d$ = 82 itd. W skrócie, zakładamy, że zaczynamy zarabiać w wieku 22 lat i od tego momentu możemy inwestować na emeryturę.
...
hm_c_I <- ggplot() +
  ...
  labs(
    title = "Heatmapa wrażliwości p w zależności od c i I",
  ...
  annotate(
    "label",
    x = Inf, y = Inf, #w prawym górnym rogu
    label = "p > 100% - poza realnym zakresem",
    hjust = 1.005, vjust = 0.8,
    fill = "darkgoldenrod",
    color = "white",
    size = 3
  )
...
zakresy_c_t <- expand.grid(c = seq(0.001, 0.09, 0.0005), t = 33:53)
...
hm_c_t <- ggplot() +
  ...
  labs(
    title = TeX("Mapa wrażliwości p w zależności od $c$ i $t$"),
...
zakresy_I_t <- expand.grid(I = seq(0.00, 0.08, 0.0005), t = 33:53)
...
hm_I_t <- ggplot() +
  ...
  labs(
    title = "Heatmapa wrażliwości p w zależności od I i t",
    x = "I (stopa wzrostu pensji w %)",
    y = "t (wiek przejścia na emeryturę)"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12, face = "bold"),
...
zakresy_c_t0 <- expand.grid(c = seq(0.02, 0.09, 0.0005), t0 = 0:23)
...
hm_c_t0 <- ggplot() +
  ...
  labs(
    ...
    x = "c (stopa zwrotu z obligacji w %)",
    y = TeX("$t_0$ (wiek rozpoczęcia oszczędzania na emeryturę)")
  ) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12, face = "bold"),
    axis.text = element_text(size = 10)
...
```{r,fig.align='center',ig.width=7,fig.height=9}
grid.arrange(hm_c_t,hm_I_t,hm_c_t0,ncol=1)
```
