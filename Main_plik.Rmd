...
wyniki_2$parametr <- factor(
  ...
  labels = c("Inflacja Pi", "Stopa zwrotu c", "Wzrost pensji I", "Rok przejścia na emeryturę", "Rok śmierci")
)

 
# ggplot(wyniki_2, aes(x = wartosc, y = p, color = parametr)) +
...
#     "Rok przejścia na emeryturę" = "forestgreen",
#     "Rok śmierci" = "firebrick"
#   )) +
...
# 
# ggsave("wykresy_wrazliwosci.png", width = 12, height = 8, dpi = 300)
#dołączam możliwość zobaczenia, że kod jest poprawny ale zapis pdf zwariował :(

# Heatmapa wpływu zwrotu z obligacji i wzrostu wypłaty na p


zakresy_hm <- expand.grid(c = seq(0.01, 0.2, 0.005), I = seq(0, 0.1, 0.005))
...
hm_c_I <- ggplot(zakresy_hm, aes(x = c, y = I, fill = p)) +
  geom_tile() +
  scale_fill_gradient(low = "skyblue", high = "maroon3") +
  labs(title = "Mapa wrażliwości p dla c i I",
       x = "c (stopa zwrotu z obligacji) ",
       y = "I (stopa wzrostu wypłaty)",
       fill = "p (%)")
...
}, zakresy_t_td$t, zakresy_t_td$td)

ggplot(zakresy_t_td, aes(x = t, y = td, fill = p)) +
  geom_tile() +
 scale_fill_viridis_c(option = "plasma") +
  labs(
    title = "Mapa wrażliwości p dla t i td",
    x = "t",
    y = "td",
    fill = "p (%)"
  )
...
#limit 100% ??
ggplot(subset(zakresy_t_td, p <= 100), aes(x = t, y = td, fill = p)) +
  geom_tile() +
  scale_fill_viridis_c(option = "plasma", limits = c(0, 100)) +
  labs(
    title = "Mapa wrażliwości p dla t i td - do 100%",
    x = "t",
    y = "td",
    fill = "p (%)"
  )
...
```{r, out.width='100%'}
knitr::include_graphics("wykresy_wrazliwosci.png")
