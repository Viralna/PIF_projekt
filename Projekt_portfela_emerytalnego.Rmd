...
# $\S_1$ Procent inwestowanej pensji p
...
pozostały kapitał powinien podlegać dziedziczeniu, tak jak ma to miejsce w IKE/IKZE.
    ...
    \item \textbf{Uwzględnić korzyści/zobowiązania emerytalne} Można uwzględnić:
      \begin{itemize}
...
r &\; \text{- realna stopa wolna od ryzyka},\\
...
tabelka_stop_WIG_1 <- kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "\\textbf{Historyczne roczne stopy zwrotu indeksu WIG}"
) %>%
  kable_styling(
...
tabelka_wartosci <- kable(
  ...
  booktabs = TRUE,
  escape = FALSE,
  caption = "\\textbf{Parametry modelu dwumianowego}"
) %>%
  kable_styling(
...
tabelka_ER <- kable(
  ...
  booktabs = TRUE,
  caption = "\\textbf{Oczekiwane roczne stopy zwrotu}"
) %>%
  kable_styling(
    latex_options = c("striped", "hold_position"),
    position = "center",
